...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="min-w-0">
            <h1 class="text-xl sm:text-2xl font-bold gradient-text">📝 Tambah Entry Jurnal</h1>
            <p class="text-gray-400 mt-1 text-sm">Catat hasil dan analisis trade Anda hari ini</p>
        </div>
    ...
    <div class="tech-card rounded-2xl p-4 sm:p-6">
        <form action="{{ route('journal.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="entry_date"
                           value="{{ old('entry_date', today()->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="text" name="currency_pair"
                           ...
                           class="form-input" placeholder="EUR/USD, XAU/USD, ...">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="trade_type" class="form-select" required>
                        ...
                        <option value="analysis" {{ old('trade_type') === 'analysis' ? 'selected' : '' }}>🔍 Analysis Only</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="result" class="form-select">
                        ...
                        <option value="breakeven" {{ old('result') === 'breakeven' ? 'selected' : '' }}>➖ Breakeven</option>
                    </select>
                </div>
            </div>

            {{-- Row 3: P&L & Market Condition --}}
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="form-label">Profit / Loss</label>
                    <input type="number" name="profit_loss"
                           value="{{ old('profit_loss', 0) }}"
                           step="0.01" class="form-input"
                           placeholder="Negatif jika loss (contoh: -50000)">
                </div>
                <div>
                    <label class="form-label">Kondisi Market</label>
                    <select name="market_condition" class="form-select">
                        <option value="">— Pilih —</option>
                        <option value="trending"  {{ old('market_condition') === 'trending'  ? 'selected' : '' }}>📈 Trending</option>
                        <option value="ranging"   {{ old('market_condition') === 'ranging'   ? 'selected' : '' }}>↔️ Ranging</option>
                        <option value="volatile"  {{ old('market_condition') === 'volatile'  ? 'selected' : '' }}>⚡ Volatile</option>
                    </select>
                </div>
            </div>
            ...
            {{-- Mood --}}
            <div class="mb-4">
                <label class="form-label">Mood Trader Hari Ini</label>
                <div class="flex gap-2">
                    @for($i = 1; $i <= 5; $i++)
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="emotion_score" value="{{ $i }}"
                               {{ old('emotion_score') == $i ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="mb-4">
                ...
                <textarea name="analysis" rows="4" class="form-input resize-none"
                    placeholder="Kenapa ambil trade ini? Setup apa yang digunakan? Entry & exit reason?">{{ old('analysis') }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="lesson_learned" rows="3" class="form-input resize-none"
                    placeholder="Apa yang bisa diperbaiki? Apa yang berhasil? Apa yang akan dilakukan berbeda?">{{ old('lesson_learned') }}</textarea>
            </div>

            <div class="flex gap-3">
                <button type="submit" class="btn-primary flex-1">
                    <i class="fas fa-save mr-2"></i>Simpan Entry Jurnal
                </button>
