...
class TradingPreview {
        ...
        preview_days.forEach(day => {
            html += `
                <tr>
                    ...
                    <td class="text-right font-mono">${day.lot_size.toFixed(4)}</td>
                </tr>
            `;
        });
    ...
    createPreviewContainer() {
        // Find the results column (lg:col-span-2) to insert preview inside it
        const resultsCol = this.form.closest('.grid')?.querySelector('.lg\\:col-span-2');
        
        const container = document.createElement('div');
        container.id = 'previewContainer';
        container.className = 'tech-card rounded-2xl p-4 sm:p-6 hidden';
        ...
        // Insert at the top of the results column
        if (resultsCol) {
            resultsCol.insertBefore(container, resultsCol.firstChild);
        } else {
            // Fallback: insert after the grid
            this.form.closest('.grid')?.parentNode.insertBefore(container, this.form.closest('.grid')?.nextSibling);
        }
